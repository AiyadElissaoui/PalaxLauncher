...
def main():
    ...
    def update_mods():
        ...
        if mods_list == fichiers:
            ...
            urllib.request.urlretrieve(url, f'{minecraft_directory}/mods/{mods_list[1]}')

            # cgm
            url = 'https://4d2a096c-ed43-4627-a525-cdcf1eacad66.usrfiles.com/archives' \
                  '/4d2a09_f5f614ba2307460b813c53265dc89a38.jar '
            urllib.request.urlretrieve(url, f'{minecraft_directory}/mods/{mods_list[0]}')
    ...
    # checkbutton
    v = IntVar()
    bouton = Checkbutton(window, command=etat, variable=v, text="Se souvenir de moi")

    # créer la zone pour rentrer l'URL code valide
    code_url = Entry(window, width=50)

    # police d'écriture
    f1 = font.Font(family='Impact', size=25, weight="bold")
    f2 = font.Font(family='Impact', size=10)
    # créer le bouton pour lancer minecraft
    launch_button = Button(window, text="JOUER", width=10, height=1, bg='green', command=launch, borderwidth=0)
